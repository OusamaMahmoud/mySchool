...
const AddStudent = () => {
  ...
  const handleCreateStudent = async () => {
    ...
    formData.append("name", studentObject?.name);
    formData.append("dateOfBirth", "2024-09-04T07:41:23.157Z");
    formData.append("gradeId", studentObject?.gradeId.toString());
    ...
    try {
      ...
      toast.success("Great job! Everything went perfectly!", {
        position: "top-right", // or "bottom-right" depending on your preference
      });
      setTimeout(() => {
        navigate("/students");
      }, 1500); // Adjust the delay (1000ms = 1 second) as needed
    } catch (error) {
      ...
      <div className="flex flex-wrap items-center max-w-3xl mx-auto  gap-x-8 gap-y-5 mt-8">
        ...
        <div className="flex justify-start items-center w-full mt-2">
          <button
            ...
            className={`btn btn-accent  ${isLoading ? "animate-pulse" : ""} `}
          >
